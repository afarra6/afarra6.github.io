...
<table id="large" style="display:none;">
  {% for row in site.data.large %}
    {% if forloop.first %}
    <tr data-sort-method="none" sytle="background-cloor:#fbe19bff">
      {% for pair in row %}
        <th>{{ pair[0] }}</th>
      {% endfor %}
    </tr>
